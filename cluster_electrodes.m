...
%
%Requirements/dependencies: eroded brain mask from T1 image and CT
...
%
%Inputs (line 33-35): sub, nEle, firstThreshold
...
%
%N.B. script is hardcoded to deal with data in /cubric/collab/seeg/ -
%this will need to be altered if working elsewhere

addpath(genpath('~/tools/iElectrodes'));
addpath(genpath('/cubric/software/spm'));
addpath(genpath('/cubric/collab/seeg/scripts'));

%Inputs - change per patient (no need to change default treshold value in
%most cases)
sub = '002';
nEle = 14;
firstThreshold = 650;

basedir = sprintf('/cubric/collab/seeg/analysis/%s/Imaging/', sub);
cd(basedir);
